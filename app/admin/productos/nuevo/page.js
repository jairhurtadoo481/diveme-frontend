...
export default function NuevoProductoPage() {
  ...
  useEffect(() => {
    ...
    if (duplicado) {
      ...
      setForm({
        ...
        precio: data.precio || "",
        categoria: data.categoria || "mujer",
        seccion: data.seccion || "parte_superior",
        tipo: data.tipo || TIPOS_POR_SECCION.parte_superior[0].valor,
  ...
  const manejarCambio = (e) => {
    ...
    if (name === "seccion") {
      setForm({ ...form, seccion: value, tipo: TIPOS_POR_SECCION[value][0].valor });
      return;
    }
  ...
  const manejarSubmit = async (e) => {
    ...
    try {
      ...
      if (video) {
        await subirVideoProducto(token, productoCreado._id, video);
      }
      ...
      <div className="bg-white min-h-screen">
        <div className="max-w-2xl mx-auto px-4 py-10">
          ...
          <form onSubmit={manejarSubmit} className="flex flex-col gap-4">
            ...
            <div>
              <p className="font-semibold mb-2 text-gray-900">Video (opcional)</p>
              <input
                type="file"
                accept="video/*"
              ...
              />
              {video && (
                <p className="text-sm text-gray-500 mt-1">Video listo: {video.name}</p>
              )}
            </div>
