...
export const subirVideoProducto = async (token, id, archivo) => {
  const formData = new FormData();
  formData.append("video", archivo);

  const res = await fetch(`${API_URL}/productos/${id}/video`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.mensaje || "Error al subir video");
  return data;
};

export const eliminarVideoProducto = async (token, id) => {
  const res = await fetch(`${API_URL}/productos/${id}/video`, {
    method: "DELETE",
    headers: { Authorization: `Bearer ${token}` },
  });
  const data = await res.json();
  if (!res.ok) throw new Error(data.mensaje || "Error al eliminar video");
  return data;
};
